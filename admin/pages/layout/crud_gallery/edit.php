...
$id = $_GET['id'];
$b = mysqli_fetch_array(mysqli_query($conect, "SELECT * FROM tb_gallery WHERE id_gallery='$id'"));
...
if (isset($_POST['input'])) {
    if (empty($foto)) {
        ...
        $qus = mysqli_query($conect, "UPDATE `tb_gallery` SET `id_gallery`='$id',`judul`='$judul' WHERE id_gallery='$id'");
        if ($qus) {
        ...
        unlink('../../../public/img/gallery/' . $gambarbaru);
        move_uploaded_file($foto, '../../../public/img/gallery/' . $newnama);

        $sql = mysqli_query($conect, "UPDATE `tb_gallery` SET `id_gallery`='$id',`judul`='$judul' , `gambar`='$newnama' WHERE id_gallery='$id'");
        if ($sql) {
            echo "<script>alert('input berhasil');document.location='../gallery.php'</script>";
        } else {
            echo "<script>alert('input gagal silahkan coba lagi');</script>";
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                <form action="" method="POST" autocomplete="off" enctype="multipart/form-data">
                                    ...
                                    <label class="mt-3">Masukan Gambar Baru</label>
