...
$id = $_GET['id'];
$b = mysqli_fetch_array(mysqli_query($conect, "SELECT * FROM tb_carousel WHERE id_carousel='$id'"));
...
if (isset($_POST['input'])) {
    ...
    if (empty($foto)) {
        ...
        $qus = mysqli_query($conect, "UPDATE `tb_carousel` SET `id_carousel`='$id',`judul`='$judul',`isi`='$isi' WHERE id_carousel='$id'");
        if ($qus) {
            echo "<script>alert('input berhasil');document.location='../carousel.php'</script>";
        } else {
            echo "<script>alert('input gagal silahkan coba lagi');</script>";
        }
    } else {
        unlink('../../../public/img/gambar_carousel/' . $gambarbaru);
        move_uploaded_file($foto, '../../../public/img/gambar_carousel/' . $newnama);

        $sql = mysqli_query($conect, "UPDATE `tb_carousel` SET `id_carousel`='$id',`judul`='$judul',`isi`='$isi', `gambar`='$newnama' WHERE id_carousel='$id'");
        if ($sql) {
            echo "<script>alert('input berhasil');document.location='../carousel.php'</script>";
        } else {
            echo "<script>alert('input gagal silahkan coba lagi');</script>";
               ...
               with font-awesome or any other icon font library -->
                        ...
                        <li class="nav-item has-treeview menu-open">
                            ...
                            <ul class="nav nav-treeview">
                                ...
                                <li class="nav-item">
                                    <a href="../../layout/carousel.php" class="nav-link active">
                                        <i class="far fa-circle nav-icon"></i>
                                        <p>Carousel</p>
                                    </a>
                                </li>
                            ...
                            <div class="card-body">
                                <form action="" method="POST" autocomplete="off" enctype="multipart/form-data">
                                    ...
                                    <div class="form-floating">
                                        <textarea maxlength="600" name="isi" class="form-control" placeholder="Text panjang max 600" id="floatingTextarea2" style="height: 100px"><?= $b['isi']; ?></textarea>
